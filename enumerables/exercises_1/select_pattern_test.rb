...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_dinosaurs
    ...
    dinosaurs = []
    animals.each {|word| dinosaurs << word if word[-5..-1] == "aurus"}
  ...
  def test_pick_arrays
    ...
    arrays = []
    elements.each {|array| arrays << array if array.is_a? Array}
  ...
  def test_pick_hashes
    ...
    hashes = []
    elements.each{|hash| hashes << hash if hash.is_a? Hash}
